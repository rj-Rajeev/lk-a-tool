...
export async function approvePostDraft(userId: number, topicId: number) {
  await db.query(
    `
    UPDATE post_drafts
...
export async function getPostDraftById(
  ...
  const [rows]: any = await db.query(
    `SELECT id, topic, content, status, is_approved, approved_at  FROM post_drafts WHERE id = ? AND status <> 'published' LIMIT 1`,
    [draftId]
  );
...
export async function getAllPostDraftTopics(userId: number) {
    const [rows] = await db.query(`SELECT id, topic, is_approved, approved_at FROM post_drafts WHERE user_id = ? AND status <> 'published'`,[userId]);
    return rows;
}
